...
int CPUTimeNextProcess = 1000;//arrival time of next process
int nextProg = 1;
...
SRTN()//start of SRTN algorithm
{
	...
	{
		if(RunProg(readyQueue[shortest]) == 0)//run process with shortest remaining time, and if it runs to completion, remove from queue
...
{
	if(nextProg < totalProgs)
		CPUTimeNextProcess = P[nextProg++].arrivalTime;
	...
	do
	{
		...
		if(CPUTimeNextProcess == CPUTime)
		{
			return 1;
		}
	} while(P[jobNum].serviceTime > 0);
